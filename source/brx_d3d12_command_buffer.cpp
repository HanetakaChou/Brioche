...
{
    mcrt_vector<D3D12_RESOURCE_BARRIER> acquire_barriers(static_cast<size_t>(storage_asset_buffer_count + sampled_asset_image_count + asset_compacted_bottom_level_acceleration_structure_count));
    ...
    {
        ID3D12Resource *asset_acceleration_structure_buffer_resource = static_cast<brx_d3d12_asset_compacted_bottom_level_acceleration_structure const *>(wrapped_asset_compacted_bottom_level_acceleration_structures[asset_compacted_bottom_level_acceleration_structure_index])->get_resource();

        // https://microsoft.github.io/DirectX-Specs/d3d/Raytracing.html#synchronizing-acceleration-structure-memory-writesreads
        acquire_barriers[storage_asset_buffer_count + sampled_asset_image_count + asset_compacted_bottom_level_acceleration_structure_index] = D3D12_RESOURCE_BARRIER{
    ...
    if (storage_asset_buffer_count > 0U || sampled_asset_image_count > 0U || asset_compacted_bottom_level_acceleration_structure_count > 0U)
    {
...
void brx_d3d12_upload_command_buffer::build_bottom_level_acceleration_structure_pass_load(uint32_t vertex_position_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *wrapped_vertex_position_buffers, uint32_t index_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *wrapped_index_buffers)
{
    if (!this->m_uma)
    {
        mcrt_vector<D3D12_RESOURCE_BARRIER> load_barriers(static_cast<size_t>(vertex_position_buffer_count + index_buffer_count));
        ...
        {
            ID3D12Resource *const unwrapped_vertex_position_buffer_resource = static_cast<brx_d3d12_acceleration_structure_build_input_read_only_buffer const *>(wrapped_vertex_position_buffers[vertex_position_buffer_index])->get_resource();

            load_barriers[vertex_position_buffer_index] = D3D12_RESOURCE_BARRIER{
                .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
                .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
                .Transition = {
        ...
        {
            ID3D12Resource *const unwrapped_index_buffer_resource = static_cast<brx_d3d12_acceleration_structure_build_input_read_only_buffer const *>(wrapped_index_buffers[index_buffer_index])->get_resource();

            load_barriers[vertex_position_buffer_count + index_buffer_index] = D3D12_RESOURCE_BARRIER{
                .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
                .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
                .Transition = {
...
void brx_d3d12_upload_command_buffer::build_bottom_level_acceleration_structure_pass_store(uint32_t vertex_position_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *wrapped_vertex_position_buffers, uint32_t index_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *wrapped_index_buffers)
{
    mcrt_vector<D3D12_RESOURCE_BARRIER> store_barriers(static_cast<size_t>(vertex_position_buffer_count + index_buffer_count));

    for (uint32_t vertex_position_buffer_index = 0U; vertex_position_buffer_index < vertex_position_buffer_count; ++vertex_position_buffer_index)
    {
        ID3D12Resource *const unwrapped_vertex_position_buffer_resource = static_cast<brx_d3d12_acceleration_structure_build_input_read_only_buffer const *>(wrapped_vertex_position_buffers[vertex_position_buffer_index])->get_resource();

        store_barriers[vertex_position_buffer_index] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .Transition = {
                unwrapped_vertex_position_buffer_resource,
    ...
    for (uint32_t index_buffer_index = 0U; index_buffer_index < index_buffer_count; ++index_buffer_index)
    {
        ...
        store_barriers[vertex_position_buffer_count + index_buffer_index] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .Transition = {
                unwrapped_index_buffer_resource,
...
{
    mcrt_vector<D3D12_RESOURCE_BARRIER> release_barriers(static_cast<size_t>((!this->m_uma) ? (storage_asset_buffer_count + sampled_asset_image_count + asset_compacted_bottom_level_acceleration_structure_count) : asset_compacted_bottom_level_acceleration_structure_count));
    ...
    for (uint32_t asset_compacted_bottom_level_acceleration_structure_index = 0U; asset_compacted_bottom_level_acceleration_structure_index < asset_compacted_bottom_level_acceleration_structure_count; ++asset_compacted_bottom_level_acceleration_structure_index)
    {
        ID3D12Resource *const unwrapped_asset_compacted_bottom_level_acceleration_structure_buffer_resource = static_cast<brx_d3d12_asset_compacted_bottom_level_acceleration_structure const *>(wrapped_asset_compacted_bottom_level_acceleration_structures[asset_compacted_bottom_level_acceleration_structure_index])->get_resource();

        // https://microsoft.github.io/DirectX-Specs/d3d/Raytracing.html#synchronizing-acceleration-structure-memory-writesreads
        release_barriers[(!this->m_uma) ? (storage_asset_buffer_count + sampled_asset_image_count + asset_compacted_bottom_level_acceleration_structure_index) : asset_compacted_bottom_level_acceleration_structure_index] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_UAV,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .UAV = {
                unwrapped_asset_compacted_bottom_level_acceleration_structure_buffer_resource}};
    }
