...
void brx_d3d12_graphics_command_buffer::acquire(uint32_t storage_asset_buffer_count, brx_storage_asset_buffer const *const *wrapped_storage_asset_buffers, uint32_t sampled_asset_image_subresource_count, BRX_SAMPLED_ASSET_IMAGE_SUBRESOURCE const *wrapped_sampled_asset_image_subresources, uint32_t compacted_bottom_level_acceleration_structure_count, brx_compacted_bottom_level_acceleration_structure const *const *wrapped_compacted_bottom_level_acceleration_structures)
{
    ...
    {
        ID3D12Resource *const storage_asset_buffer_resource = static_cast<brx_d3d12_storage_asset_buffer const *>(wrapped_storage_asset_buffers[storage_asset_buffer_index])->get_resource();
...
void brx_d3d12_graphics_command_buffer::begin_render_pass(brx_render_pass const *brx_render_pass, brx_frame_buffer const *brx_frame_buffer, uint32_t width, uint32_t height, uint32_t color_clear_value_count, float const (*color_clear_values)[4], float const *depth_clear_value, uint8_t const *stencil_clear_value)
{
    ...
    mcrt_vector<D3D12_RESOURCE_BARRIER> load_barriers(static_cast<size_t>(color_attachment_flush_for_sampled_image_count + color_attachment_flush_for_present_count + (depth_stencil_attachment_flush_for_sampled_image ? 1U : 0U)));
    ...
    for (uint32_t flush_for_sampled_image_index = 0U; flush_for_sampled_image_index < color_attachment_flush_for_sampled_image_count; ++flush_for_sampled_image_index)
    {
        ...
        load_barriers[flush_for_sampled_image_index] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .Transition = {
    ...
    for (uint32_t flush_for_present_index = 0U; flush_for_present_index < color_attachment_flush_for_present_count; ++flush_for_present_index)
    {
        ...
        load_barriers[color_attachment_flush_for_sampled_image_count + flush_for_present_index] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .Transition = {
                render_target_resource,
    ...
    {
        load_barriers[color_attachment_flush_for_sampled_image_count + color_attachment_flush_for_present_count] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .Transition = {
...
void brx_d3d12_graphics_command_buffer::set_scissor(int32_t offset_width, int32_t offset_height, uint32_t width, uint32_t height)
{
...
void brx_d3d12_graphics_command_buffer::draw(uint32_t vertex_count, uint32_t instance_count, uint32_t first_vertex, uint32_t first_instance)
{
...
void brx_d3d12_graphics_command_buffer::end_render_pass()
{
    ...
    mcrt_vector<D3D12_RESOURCE_BARRIER> store_barriers(static_cast<size_t>(color_attachment_flush_for_sampled_image_count + color_attachment_flush_for_present_count + (depth_stencil_attachment_flush_for_sampled_image ? 1U : 0U)));
    ...
    for (uint32_t flush_for_sampled_image_index = 0U; flush_for_sampled_image_index < color_attachment_flush_for_sampled_image_count; ++flush_for_sampled_image_index)
    {
        ...
        store_barriers[flush_for_sampled_image_index] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .Transition = {
                render_target_resource,
    ...
    for (uint32_t flush_for_present_index = 0U; flush_for_present_index < color_attachment_flush_for_present_count; ++flush_for_present_index)
    {
        ...
        store_barriers[color_attachment_flush_for_sampled_image_count + flush_for_present_index] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .Transition = {
                render_target_resource,
    ...
    if (depth_stencil_attachment_flush_for_sampled_image)
    {
        store_barriers[color_attachment_flush_for_sampled_image_count + color_attachment_flush_for_present_count] = D3D12_RESOURCE_BARRIER{
            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
            .Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE,
            .Transition = {
                depth_stencil_resource,
...
void brx_d3d12_graphics_command_buffer::compute_pass_load(uint32_t storage_buffer_count, brx_storage_buffer const *const *wrapped_storage_buffers, BRX_COMPUTE_PASS_STORAGE_BUFFER_LOAD_OPERATION const *storage_buffer_load_operations, uint32_t storage_image_count, brx_storage_image const *const *wrapped_storage_images, BRX_COMPUTE_PASS_STORAGE_IMAGE_LOAD_OPERATION const *storage_image_load_operations)
{
    // TODO: Enhanced Barriers
    // https://microsoft.github.io/DirectX-Specs/d3d/D3D12EnhancedBarriers.html
    // D3D12_BARRIER_LAYOUT_UNDEFINED
    ...
    {
        ID3D12Resource *const storage_buffer_resource = static_cast<brx_d3d12_storage_buffer const *>(wrapped_storage_buffers[storage_buffer_index])->get_resource();

        assert(BRX_COMPUTE_PASS_STORAGE_BUFFER_LOAD_OPERATION_DONT_CARE == storage_buffer_load_operations[storage_buffer_index]);
    ...
    {
        ID3D12Resource *const storage_image_resource = static_cast<brx_d3d12_storage_image const *>(wrapped_storage_images[storage_image_index])->get_resource();

        assert(BRX_COMPUTE_PASS_STORAGE_IMAGE_LOAD_OPERATION_DONT_CARE == storage_image_load_operations[storage_image_index]);
...
void brx_d3d12_graphics_command_buffer::compute_pass_store(uint32_t storage_buffer_count, brx_storage_buffer const *const *wrapped_storage_buffers, BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION const *storage_buffer_store_operations, uint32_t storage_image_count, brx_storage_image const *const *wrapped_storage_images, BRX_COMPUTE_PASS_STORAGE_IMAGE_STORE_OPERATION const *storage_image_store_operations)
{
    ...
    for (uint32_t storage_buffer_index = 0U; storage_buffer_index < storage_buffer_count; ++storage_buffer_index)
    {
        ...
        assert(BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION_FLUSH_FOR_READ_ONLY_STORAGE_BUFFER_AND_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BUFFER == storage_buffer_store_operations[storage_buffer_index]);
    ...
    for (uint32_t storage_image_index = 0U; storage_image_index < storage_image_count; ++storage_image_index)
    {
        ...
        assert(BRX_COMPUTE_PASS_STORAGE_IMAGE_STORE_OPERATION_FLUSH_FOR_SAMPLED_IMAGE == storage_image_store_operations[storage_image_index]);
...
void brx_d3d12_graphics_command_buffer::build_intermediate_bottom_level_acceleration_structure(brx_intermediate_bottom_level_acceleration_structure *wrapped_intermediate_bottom_level_acceleration_structure, uint32_t bottom_level_acceleration_structure_geometry_count, BRX_BOTTOM_LEVEL_ACCELERATION_STRUCTURE_GEOMETRY const *wrapped_bottom_level_acceleration_structure_geometries, brx_scratch_buffer *wrapped_scratch_buffer)
{
    assert(NULL != wrapped_intermediate_bottom_level_acceleration_structure);
    D3D12_GPU_VIRTUAL_ADDRESS const destination_acceleration_structure_device_memory_range_base = static_cast<brx_d3d12_intermediate_bottom_level_acceleration_structure *>(wrapped_intermediate_bottom_level_acceleration_structure)->get_resource()->GetGPUVirtualAddress();
    ...
    for (uint32_t bottom_level_acceleration_structure_geometry_index = 0U; bottom_level_acceleration_structure_geometry_index < bottom_level_acceleration_structure_geometry_count; ++bottom_level_acceleration_structure_geometry_index)
    {
        ...
        ID3D12Resource *const unwrapped_vertex_position_buffer_resource = static_cast<brx_d3d12_acceleration_structure_build_input_read_only_buffer const *>(wrapped_bottom_level_acceleration_structure_geometry.vertex_position_buffer)->get_resource();

        ID3D12Resource *const unwrapped_index_buffer_resource = (BRX_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? static_cast<brx_d3d12_acceleration_structure_build_input_read_only_buffer const *>(wrapped_bottom_level_acceleration_structure_geometry.index_buffer)->get_resource() : NULL;
    ...
    static_cast<brx_d3d12_intermediate_bottom_level_acceleration_structure *>(wrapped_intermediate_bottom_level_acceleration_structure)->set_bottom_level_acceleration_structure_geometries(bottom_level_acceleration_structure_geometry_count, wrapped_bottom_level_acceleration_structure_geometries);
}

void brx_d3d12_graphics_command_buffer::build_intermediate_bottom_level_acceleration_structure_store(uint32_t intermediate_bottom_level_acceleration_structure_count, brx_intermediate_bottom_level_acceleration_structure const *const *wrapped_intermediate_bottom_level_acceleration_structures)
{
    ...
    {
        ID3D12Resource *const unwrapped_acceleration_structure_buffer_resource = static_cast<brx_d3d12_intermediate_bottom_level_acceleration_structure const *>(wrapped_intermediate_bottom_level_acceleration_structures[intermediate_bottom_level_acceleration_structure_index])->get_resource();
...
void brx_d3d12_graphics_command_buffer::update_intermediate_bottom_level_acceleration_structure(brx_intermediate_bottom_level_acceleration_structure *wrapped_intermediate_bottom_level_acceleration_structure, brx_acceleration_structure_build_input_read_only_buffer const *const *wrapped_bottom_level_acceleration_structure_geometry_vertex_position_buffers, brx_scratch_buffer *wrapped_scratch_buffer)
{
    assert(NULL != wrapped_intermediate_bottom_level_acceleration_structure);
    D3D12_GPU_VIRTUAL_ADDRESS const destination_acceleration_structure_device_memory_range_base = static_cast<brx_d3d12_intermediate_bottom_level_acceleration_structure *>(wrapped_intermediate_bottom_level_acceleration_structure)->get_resource()->GetGPUVirtualAddress();
    assert(0U == (destination_acceleration_structure_device_memory_range_base % D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT));

    mcrt_vector<BRX_BOTTOM_LEVEL_ACCELERATION_STRUCTURE_GEOMETRY> const &wrapped_bottom_level_acceleration_structure_geometries = static_cast<brx_d3d12_intermediate_bottom_level_acceleration_structure *>(wrapped_intermediate_bottom_level_acceleration_structure)->get_bottom_level_acceleration_structure_geometries();
    ...
    for (uint32_t bottom_level_acceleration_structure_geometry_index = 0U; bottom_level_acceleration_structure_geometry_index < bottom_level_acceleration_structure_geometry_count; ++bottom_level_acceleration_structure_geometry_index)
    {
        ...
        ID3D12Resource *const unwrapped_vertex_position_buffer_resource = static_cast<brx_d3d12_acceleration_structure_build_input_read_only_buffer const *>(wrapped_bottom_level_acceleration_structure_geometry_vertex_position_buffers[bottom_level_acceleration_structure_geometry_index])->get_resource();
...
void brx_d3d12_graphics_command_buffer::update_intermediate_bottom_level_acceleration_structure_store(uint32_t intermediate_bottom_level_acceleration_structure_count, brx_intermediate_bottom_level_acceleration_structure const *const *wrapped_intermediate_bottom_level_acceleration_structures)
{
    mcrt_vector<D3D12_RESOURCE_BARRIER> store_barriers(static_cast<size_t>(intermediate_bottom_level_acceleration_structure_count));
...
void brx_d3d12_graphics_command_buffer::build_top_level_acceleration_structure_store(brx_top_level_acceleration_structure *wrapped_top_level_acceleration_structure)
{
    assert(NULL != wrapped_top_level_acceleration_structure);
    ID3D12Resource *const unwrapped_acceleration_structure_buffer_resource = static_cast<brx_d3d12_top_level_acceleration_structure *>(wrapped_top_level_acceleration_structure)->get_resource();
...
void brx_d3d12_graphics_command_buffer::update_top_level_acceleration_structure_store(brx_top_level_acceleration_structure *wrapped_top_level_acceleration_structure)
{
    assert(NULL != wrapped_top_level_acceleration_structure);
    ID3D12Resource *const destination_acceleration_structure_buffer_resource = static_cast<brx_d3d12_top_level_acceleration_structure *>(wrapped_top_level_acceleration_structure)->get_resource();
...
void brx_d3d12_upload_command_buffer::upload_from_staging_upload_buffer_to_storage_asset_buffer(brx_storage_asset_buffer *wrapped_storage_asset_buffer, uint64_t dst_offset, brx_staging_upload_buffer *wrapped_staging_upload_buffer, uint64_t src_offset, uint32_t src_size)
{
    ID3D12Resource *asset_buffer_resource = static_cast<brx_d3d12_storage_asset_buffer *>(wrapped_storage_asset_buffer)->get_resource();
...
void brx_d3d12_upload_command_buffer::upload_from_staging_upload_buffer_to_sampled_asset_image(brx_sampled_asset_image *wrapped_sampled_asset_image, BRX_SAMPLED_ASSET_IMAGE_FORMAT wrapped_sampled_asset_image_format, uint32_t sampled_asset_image_width, uint32_t sampled_asset_image_height, uint32_t dst_mip_level, brx_staging_upload_buffer *wrapped_staging_upload_buffer, uint64_t src_offset, uint32_t src_row_pitch, uint32_t src_row_count)
{
    assert(NULL != wrapped_sampled_asset_image);
    ID3D12Resource *const sampled_asset_image = static_cast<brx_d3d12_sampled_asset_image *>(wrapped_sampled_asset_image)->get_resource();
    ...
    uint32_t const block_width = brx_get_format_block_width(wrapped_sampled_asset_image_format);
    uint32_t const block_height = brx_get_format_block_height(wrapped_sampled_asset_image_format);
...
void brx_d3d12_upload_command_buffer::build_non_compacted_bottom_level_acceleration_structure_pass_load(uint32_t acceleration_structure_build_input_read_only_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *wrapped_acceleration_structure_build_input_read_only_buffers)
{
    if (!this->m_uma)
    {
        ...
        {
            ID3D12Resource *const unwrapped_acceleration_structure_build_input_read_only_buffer_resource = static_cast<brx_d3d12_acceleration_structure_build_input_read_only_buffer const *>(wrapped_acceleration_structure_build_input_read_only_buffers[acceleration_structure_build_input_read_only_buffer_index])->get_resource();
...
void brx_d3d12_upload_command_buffer::build_non_compacted_bottom_level_acceleration_structure(brx_non_compacted_bottom_level_acceleration_structure *wrapped_non_compacted_bottom_level_acceleration_structure, uint32_t bottom_level_acceleration_structure_geometry_count, BRX_BOTTOM_LEVEL_ACCELERATION_STRUCTURE_GEOMETRY const *wrapped_bottom_level_acceleration_structure_geometries, brx_scratch_buffer *wrapped_scratch_buffer, brx_compacted_bottom_level_acceleration_structure_size_query_pool *wrapped_compacted_bottom_level_acceleration_structure_size_query_pool, uint32_t query_index)
{
    assert(NULL != wrapped_non_compacted_bottom_level_acceleration_structure);
    D3D12_GPU_VIRTUAL_ADDRESS const destination_acceleration_structure_device_memory_range_base = static_cast<brx_d3d12_non_compacted_bottom_level_acceleration_structure *>(wrapped_non_compacted_bottom_level_acceleration_structure)->get_resource()->GetGPUVirtualAddress();
    assert(0U == (destination_acceleration_structure_device_memory_range_base % D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT));
...
void brx_d3d12_upload_command_buffer::build_non_compacted_bottom_level_acceleration_structure_pass_store(uint32_t acceleration_structure_build_input_read_only_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *wrapped_acceleration_structure_build_input_read_only_buffers)
{
    if (!this->m_uma)
    {
...
void brx_d3d12_upload_command_buffer::compact_bottom_level_acceleration_structure(brx_compacted_bottom_level_acceleration_structure *wrapped_destination_compacted_bottom_level_acceleration_structure, brx_non_compacted_bottom_level_acceleration_structure *wrapped_source_non_compacted_bottom_level_acceleration_structure)
{
    ...
    ID3D12Resource *const destination_acceleration_structure_buffer_resource = static_cast<brx_d3d12_compacted_bottom_level_acceleration_structure *>(wrapped_destination_compacted_bottom_level_acceleration_structure)->get_resource();
    D3D12_GPU_VIRTUAL_ADDRESS const destination_acceleration_structure_device_memory_range_base = destination_acceleration_structure_buffer_resource->GetGPUVirtualAddress();
    assert(0U == (destination_acceleration_structure_device_memory_range_base % D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT));
    D3D12_GPU_VIRTUAL_ADDRESS const source_acceleration_structure_device_memory_range_base = static_cast<brx_d3d12_non_compacted_bottom_level_acceleration_structure *>(wrapped_source_non_compacted_bottom_level_acceleration_structure)->get_resource()->GetGPUVirtualAddress();
...
void brx_d3d12_upload_command_buffer::release(uint32_t storage_asset_buffer_count, brx_storage_asset_buffer const *const *wrapped_storage_asset_buffers, uint32_t sampled_asset_image_subresource_count, BRX_SAMPLED_ASSET_IMAGE_SUBRESOURCE const *wrapped_sampled_asset_image_subresources, uint32_t compacted_bottom_level_acceleration_structure_count, brx_compacted_bottom_level_acceleration_structure const *const *wrapped_compacted_bottom_level_acceleration_structures)
{
    ...
    if (!this->m_uma)
    {
        for (uint32_t storage_asset_buffer_index = 0U; storage_asset_buffer_index < storage_asset_buffer_count; ++storage_asset_buffer_index)
        {
            ID3D12Resource *const asset_buffer_resource = static_cast<brx_d3d12_storage_asset_buffer const *>(wrapped_storage_asset_buffers[storage_asset_buffer_index])->get_resource();
