...
brx_d3d12_frame_buffer::brx_d3d12_frame_buffer(mcrt_vector<ID3D12Resource *> &&render_target_resources, mcrt_vector<D3D12_CPU_DESCRIPTOR_HANDLE> &&render_target_view_descriptors, ID3D12Resource *depth_stencil_resource, mcrt_vector<D3D12_CPU_DESCRIPTOR_HANDLE> &&depth_stencil_view_descriptor) : m_render_target_resources(std::move(render_target_resources)), m_render_target_view_descriptors(std::move(render_target_view_descriptors)), m_depth_stencil_resource(depth_stencil_resource), m_depth_stencil_view_descriptor(std::move(depth_stencil_view_descriptor))
{
