...
brx_d3d12_color_attachment_intermediate_image::brx_d3d12_color_attachment_intermediate_image() : m_resource(NULL), m_allocation(NULL), m_render_target_view_descriptor_heap(NULL), m_render_target_view_descriptor{0U}
{
}

void brx_d3d12_color_attachment_intermediate_image::init(ID3D12Device *device, D3D12MA::Allocator *memory_allocator, D3D12MA::Pool *color_attachment_intermediate_image_memory_pool, BRX_COLOR_ATTACHMENT_IMAGE_FORMAT wrapped_color_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
...
void brx_d3d12_color_attachment_intermediate_image::uninit()
{
	assert(NULL != this->m_render_target_view_descriptor_heap);
...
brx_d3d12_color_attachment_intermediate_image::~brx_d3d12_color_attachment_intermediate_image()
{
	assert(NULL == this->m_render_target_view_descriptor_heap);
	assert(NULL == this->m_resource);
	assert(NULL == this->m_allocation);
...
ID3D12Resource *brx_d3d12_color_attachment_intermediate_image::get_resource() const
{
	return this->m_resource;
}

D3D12_CPU_DESCRIPTOR_HANDLE brx_d3d12_color_attachment_intermediate_image::get_render_target_view_descriptor() const
{
	return this->m_render_target_view_descriptor;
}

D3D12_SHADER_RESOURCE_VIEW_DESC const *brx_d3d12_color_attachment_intermediate_image::get_shader_resource_view_desc() const
{
	return &this->m_shader_resource_view_desc;
}

brx_sampled_image const *brx_d3d12_color_attachment_intermediate_image::get_sampled_image() const
{
...
brx_d3d12_depth_stencil_attachment_intermediate_image::brx_d3d12_depth_stencil_attachment_intermediate_image() : m_resource(NULL), m_allocation(NULL), m_depth_stencil_view_descriptor_heap(NULL), m_depth_stencil_view_descriptor{0U}
{
}

void brx_d3d12_depth_stencil_attachment_intermediate_image::init(ID3D12Device *device, D3D12MA::Allocator *memory_allocator, D3D12MA::Pool *depth_stencil_attachment_intermediate_image_memory_pool, BRX_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT wrapped_depth_stencil_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
...
void brx_d3d12_depth_stencil_attachment_intermediate_image::uninit()
{
	assert(NULL != this->m_depth_stencil_view_descriptor_heap);
...
brx_d3d12_depth_stencil_attachment_intermediate_image::~brx_d3d12_depth_stencil_attachment_intermediate_image()
{
	assert(NULL == this->m_depth_stencil_view_descriptor_heap);
	assert(NULL == this->m_resource);
	assert(NULL == this->m_allocation);
...
ID3D12Resource *brx_d3d12_depth_stencil_attachment_intermediate_image::get_resource() const
{
	return this->m_resource;
}

D3D12_CPU_DESCRIPTOR_HANDLE brx_d3d12_depth_stencil_attachment_intermediate_image::get_depth_stencil_view_descriptor() const
{
	return this->m_depth_stencil_view_descriptor;
}

D3D12_SHADER_RESOURCE_VIEW_DESC const *brx_d3d12_depth_stencil_attachment_intermediate_image::get_shader_resource_view_desc() const
{
	return &this->m_shader_resource_view_desc;
}

brx_sampled_image const *brx_d3d12_depth_stencil_attachment_intermediate_image::get_sampled_image() const
{
	assert(D3D12_SRV_DIMENSION_TEXTURE2D == this->m_shader_resource_view_desc.ViewDimension);
	return static_cast<brx_d3d12_sampled_image const *>(this);
}

brx_d3d12_storage_intermediate_image::brx_d3d12_storage_intermediate_image() : m_resource(NULL), m_allocation(NULL)
{
}

void brx_d3d12_storage_intermediate_image::init(D3D12MA::Allocator *memory_allocator, D3D12MA::Pool *storage_intermediate_image_memory_pool, DXGI_FORMAT unwrapped_storage_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
	D3D12MA::ALLOCATION_DESC allocation_desc = {
		D3D12MA::ALLOCATION_FLAG_NONE,
		D3D12_HEAP_TYPE_CUSTOM,
...
void brx_d3d12_storage_intermediate_image::uninit()
{
	assert(NULL != this->m_resource);
	this->m_resource->Release();
	this->m_resource = NULL;
...
brx_d3d12_storage_intermediate_image::~brx_d3d12_storage_intermediate_image()
{
	assert(NULL == this->m_resource);
	assert(NULL == this->m_allocation);
}

ID3D12Resource *brx_d3d12_storage_intermediate_image::get_resource() const
{
	return this->m_resource;
}

D3D12_UNORDERED_ACCESS_VIEW_DESC const *brx_d3d12_storage_intermediate_image::get_unordered_access_view_desc() const
{
	return &this->m_unordered_access_view_desc;
}

D3D12_SHADER_RESOURCE_VIEW_DESC const *brx_d3d12_storage_intermediate_image::get_shader_resource_view_desc() const
{
	return &this->m_shader_resource_view_desc;
}

brx_sampled_image const *brx_d3d12_storage_intermediate_image::get_sampled_image() const
{
	assert(D3D12_SRV_DIMENSION_TEXTURE2D == this->m_shader_resource_view_desc.ViewDimension);
	return static_cast<brx_d3d12_sampled_image const *>(this);
}

brx_d3d12_sampled_asset_image::brx_d3d12_sampled_asset_image() : m_resource(NULL), m_allocation(NULL), m_mip_levels(static_cast<uint32_t>(-1))
{
}

void brx_d3d12_sampled_asset_image::init(bool uma, D3D12MA::Allocator *memory_allocator, D3D12MA::Pool *sampled_asset_image_memory_pool, DXGI_FORMAT unwrapped_sampled_asset_image_format, uint32_t width, uint32_t height, uint32_t unwrapped_mip_levels)
{
...
void brx_d3d12_sampled_asset_image::uninit()
{
	assert(NULL != this->m_resource);
	this->m_resource->Release();
	this->m_resource = NULL;
...
brx_d3d12_sampled_asset_image::~brx_d3d12_sampled_asset_image()
{
	assert(NULL == this->m_resource);
	assert(NULL == this->m_allocation);
}

ID3D12Resource *brx_d3d12_sampled_asset_image::get_resource() const
{
	return this->m_resource;
}

D3D12_SHADER_RESOURCE_VIEW_DESC const *brx_d3d12_sampled_asset_image::get_shader_resource_view_desc() const
{
	return &this->m_shader_resource_view_desc;
}

brx_sampled_image const *brx_d3d12_sampled_asset_image::get_sampled_image() const
{
	return static_cast<brx_d3d12_sampled_image const *>(this);
}

uint32_t brx_d3d12_sampled_asset_image::get_mip_levels() const
{
