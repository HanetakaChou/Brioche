...
{
	bool operator()(brx_pal_d3d12_pipeline_set const &lhs, brx_pal_d3d12_pipeline_set const &rhs) const
	{
		size_t const size = sizeof(brx_pal_d3d12_descriptor_set const *) * std::min(lhs.m_descriptor_sets.size(), rhs.m_descriptor_sets.size());
		if (size > 0U)
		{
			assert((!lhs.m_descriptor_sets.empty()) && (!rhs.m_descriptor_sets.empty()));

			int const compare = std::memcmp(lhs.m_descriptor_sets.data(), rhs.m_descriptor_sets.data(), size);
