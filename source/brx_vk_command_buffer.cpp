...
void brx_vk_graphics_command_buffer::acquire(uint32_t storage_asset_buffer_count, brx_storage_asset_buffer const *const *wrapped_storage_asset_buffers, uint32_t sampled_asset_image_count, brx_sampled_asset_image const *const *wrapped_sampled_asset_images, uint32_t const *dst_mip_levels, uint32_t asset_compacted_bottom_level_acceleration_structure_count, brx_asset_compacted_bottom_level_acceleration_structure const *const *wrapped_asset_compacted_bottom_level_acceleration_structures)
{
	if (this->m_has_dedicated_upload_queue)
	{
		if (this->m_upload_queue_family_index != this->m_graphics_queue_family_index)
		{
			...
			mcrt_vector<VkBufferMemoryBarrier> acceleration_structure_acquire_barriers(static_cast<size_t>(asset_compacted_bottom_level_acceleration_structure_count));
			...
			for (uint32_t asset_compacted_bottom_level_acceleration_structure_index = 0U; asset_compacted_bottom_level_acceleration_structure_index < asset_compacted_bottom_level_acceleration_structure_count; ++asset_compacted_bottom_level_acceleration_structure_index)
			{
				VkBuffer const asset_acceleration_structure_buffer = static_cast<brx_vk_asset_compacted_bottom_level_acceleration_structure const *>(wrapped_asset_compacted_bottom_level_acceleration_structures[asset_compacted_bottom_level_acceleration_structure_index])->get_buffer();

				acceleration_structure_acquire_barriers[asset_compacted_bottom_level_acceleration_structure_index] = VkBufferMemoryBarrier{
					VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
					NULL,
					0U,
			...
			if (storage_asset_buffer_count > 0U || sampled_asset_image_count > 0U)
			{
				...
				this->m_pfn_cmd_pipeline_barrier(this->m_command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, graphics_queue_family_store_destination_stage, 0U, 0U, NULL, static_cast<uint32_t>(buffer_acquire_barriers.size()), buffer_acquire_barriers.data(), static_cast<uint32_t>(image_acquire_barriers.size()), image_acquire_barriers.data());
			}

			if (asset_compacted_bottom_level_acceleration_structure_count > 0U)
			{
...
void brx_vk_graphics_command_buffer::compute_pass_store(uint32_t storage_buffer_count, brx_storage_buffer const *const *wrapped_storage_buffers, BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION const *storage_buffer_store_operations, uint32_t storage_image_count, brx_storage_image const *const *wrapped_storage_images, BRX_COMPUTE_PASS_STORAGE_IMAGE_STORE_OPERATION const *storage_image_store_operations)
{
	...
	for (uint32_t storage_buffer_index = 0U; storage_buffer_index < storage_buffer_count; ++storage_buffer_index)
	{
		...
		assert(BRX_COMPUTE_PASS_STORAGE_BUFFER_STORE_OPERATION_FLUSH_FOR_READ_ONLY_STORAGE_BUFFER == storage_buffer_store_operations[storage_buffer_index]);
...
void brx_vk_graphics_command_buffer::build_top_level_acceleration_structure(brx_top_level_acceleration_structure *wrapped_top_level_acceleration_structure, uint32_t top_level_acceleration_structure_instance_count, brx_top_level_acceleration_structure_instance_upload_buffer *wrapped_top_level_acceleration_structure_instance_upload_buffer, brx_scratch_buffer *wrapped_scratch_buffer)
{
	...
	VkBufferMemoryBarrier const release_barrier = {
		...
		0U,
		VK_WHOLE_SIZE};
	this->m_pfn_cmd_pipeline_barrier(this->m_command_buffer, VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR, VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR, 0U, 0U, NULL, 1U, &release_barrier, 0U, NULL);
}
...
void brx_vk_graphics_command_buffer::acceleration_structure_pass_store_top_level(brx_top_level_acceleration_structure *wrapped_top_level_acceleration_structure)
{
	...
	VkBufferMemoryBarrier const release_barrier = {
		...
		0U,
		VK_WHOLE_SIZE};
	this->m_pfn_cmd_pipeline_barrier(this->m_command_buffer, VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR, g_graphics_queue_family_all_supported_shader_read_stages | VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, 0U, 0U, NULL, 1U, &release_barrier, 0U, NULL);
}
...
void brx_vk_upload_command_buffer::build_bottom_level_acceleration_structure_pass_load(uint32_t vertex_position_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *vertex_position_buffers, uint32_t index_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *index_buffers)
{
	mcrt_vector<VkBufferMemoryBarrier> load_barriers(static_cast<size_t>(vertex_position_buffer_count + index_buffer_count));

	for (uint32_t vertex_position_buffer_index = 0U; vertex_position_buffer_index < vertex_position_buffer_count; ++vertex_position_buffer_index)
	{
		brx_vk_acceleration_structure_build_input_read_only_buffer const *const unwrapped_vertex_position_buffer = static_cast<brx_vk_acceleration_structure_build_input_read_only_buffer const *>(vertex_position_buffers[vertex_position_buffer_index]);

		load_barriers[vertex_position_buffer_index] = VkBufferMemoryBarrier{
			VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
			NULL,
			VK_ACCESS_TRANSFER_WRITE_BIT,
			VK_ACCESS_SHADER_READ_BIT,
	...
	for (uint32_t index_buffer_index = 0U; index_buffer_index < index_buffer_count; ++index_buffer_index)
	{
		brx_vk_acceleration_structure_build_input_read_only_buffer const *const unwrapped_index_buffer = static_cast<brx_vk_acceleration_structure_build_input_read_only_buffer const *>(index_buffers[index_buffer_index]);

		load_barriers[vertex_position_buffer_count + index_buffer_index] = VkBufferMemoryBarrier{
			VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
			NULL,
			VK_ACCESS_TRANSFER_WRITE_BIT,
			VK_ACCESS_SHADER_READ_BIT,
...
void brx_vk_upload_command_buffer::build_bottom_level_acceleration_structure_pass_store(uint32_t vertex_position_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *vertex_position_buffers, uint32_t index_buffer_count, brx_acceleration_structure_build_input_read_only_buffer const *const *index_buffers)
{
...
void brx_vk_upload_command_buffer::release(uint32_t storage_asset_buffer_count, brx_storage_asset_buffer const *const *wrapped_storage_asset_buffers, uint32_t sampled_asset_image_count, brx_sampled_asset_image const *const *wrapped_sampled_asset_images, uint32_t const *dst_mip_levels, uint32_t asset_compacted_bottom_level_acceleration_structure_count, brx_asset_compacted_bottom_level_acceleration_structure const *const *wrapped_asset_compacted_bottom_level_acceleration_structures)
{
	...
	mcrt_vector<VkBufferMemoryBarrier> upload_queue_family_acceleration_structure_release_barriers(static_cast<size_t>(asset_compacted_bottom_level_acceleration_structure_count));
	mcrt_vector<VkBufferMemoryBarrier> graphics_queue_family_acceleration_structure_release_barriers(static_cast<size_t>(asset_compacted_bottom_level_acceleration_structure_count));
	...
	for (uint32_t asset_compacted_bottom_level_acceleration_structure_index = 0U; asset_compacted_bottom_level_acceleration_structure_index < asset_compacted_bottom_level_acceleration_structure_count; ++asset_compacted_bottom_level_acceleration_structure_index)
	{
		...
		upload_queue_family_acceleration_structure_release_barriers[asset_compacted_bottom_level_acceleration_structure_index] = VkBufferMemoryBarrier{
			VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
			NULL,
			VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR,
			0U,
		...
		graphics_queue_family_acceleration_structure_release_barriers[asset_compacted_bottom_level_acceleration_structure_index] = VkBufferMemoryBarrier{
			VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
			NULL,
			VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR,
			VK_ACCESS_ACCELERATION_STRUCTURE_READ_BIT_KHR,
	...
	VkPipelineStageFlags const graphics_queue_family_buffer_image_release_destination_stage = (!this->m_support_ray_tracing) ? (g_graphics_queue_family_all_supported_shader_read_stages) : (g_graphics_queue_family_all_supported_shader_read_stages | VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR);
