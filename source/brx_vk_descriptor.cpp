...
void brx_vk_descriptor_set_layout::init(uint32_t support_ray_tracing, uint32_t descriptor_set_binding_count, BRX_DESCRIPTOR_SET_LAYOUT_BINDING const *wrapped_descriptor_set_bindings, uint32_t max_per_stage_descriptor_storage_buffers, uint32_t max_per_stage_descriptor_sampled_images, uint32_t max_descriptor_set_storage_buffers, uint32_t max_descriptor_set_sampled_images, PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
    ...
    for (uint32_t binding_index = 0U; binding_index < descriptor_set_binding_count; ++binding_index)
    {
        ...
        case BRX_DESCRIPTOR_TYPE_READ_ONLY_STORAGE_BUFFER:
        {
            ...
            if (BRX_DESCRIPTOR_SET_LAYOUT_BINDING_DESCRIPTOR_COUNT_UNBOUNDED != wrapped_descriptor_set_bindings[binding_index].descriptor_count)
            {
        ...
        {
            descriptor_set_bindings[binding_index].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
            assert(BRX_DESCRIPTOR_SET_LAYOUT_BINDING_DESCRIPTOR_COUNT_UNBOUNDED != wrapped_descriptor_set_bindings[binding_index].descriptor_count);
            descriptor_set_bindings[binding_index].descriptorCount = wrapped_descriptor_set_bindings[binding_index].descriptor_count;
            this->m_storage_buffer_descriptor_count += wrapped_descriptor_set_bindings[binding_index].descriptor_count;
            binding_flags[binding_index] = 0U;
        }
...
VkDescriptorType brx_vk_descriptor_set_layout::get_unbounded_descriptor_type() const
{
...
void brx_vk_descriptor_set::init(bool support_ray_tracing, brx_descriptor_set_layout const *wrapped_descriptor_set_layout, uint32_t unbounded_descriptor_count, PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
...
void brx_vk_descriptor_set::write_descriptor(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, uint32_t dst_binding, BRX_DESCRIPTOR_TYPE wrapped_descriptor_type, uint32_t dst_descriptor_start_index, uint32_t src_descriptor_count, brx_uniform_upload_buffer const *const *src_dynamic_uniform_buffers, uint32_t const *src_dynamic_uniform_buffer_ranges, brx_read_only_storage_buffer const *const *src_read_only_storage_buffers, brx_storage_buffer const *const *src_storage_buffers, brx_sampled_image const *const *src_sampled_images, brx_storage_image const *const *src_storage_images, brx_sampler const *const *src_samplers, brx_top_level_acceleration_structure const *const *src_top_level_acceleration_structures)
{
    ...
    case BRX_DESCRIPTOR_TYPE_READ_ONLY_STORAGE_BUFFER:
    {
        ...
        for (uint32_t descriptor_index = 0U; descriptor_index < src_descriptor_count; ++descriptor_index)
        {
            ...
            buffer_info[descriptor_index].buffer = static_cast<brx_vk_read_only_storage_buffer const *>(src_read_only_storage_buffers[descriptor_index])->get_buffer();
            buffer_info[descriptor_index].offset = 0U;
            buffer_info[descriptor_index].range = static_cast<brx_vk_read_only_storage_buffer const *>(src_read_only_storage_buffers[descriptor_index])->get_size();
        }
    }
    break;
    case BRX_DESCRIPTOR_TYPE_STORAGE_BUFFER:
