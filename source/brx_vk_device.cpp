...
#if defined(__linux__)
#if defined(__ANDROID__)
        char const *const dynamic_library_vulkan_file_name = "libvulkan.so";
#else
        char const *const dynamic_library_vulkan_file_name = "libvulkan.so.1";
#endif
        this->m_dynamic_library_vulkan = dlopen(dynamic_library_vulkan_file_name, RTLD_NOW | RTLD_LOCAL);
        assert(NULL != this->m_dynamic_library_vulkan);
...
brx_graphics_pipeline *brx_vk_device::create_graphics_pipeline(brx_render_pass const *render_pass, brx_pipeline_layout const *pipeline_layout, size_t vertex_shader_module_code_size, void const *vertex_shader_module_code, size_t fragment_shader_module_code_size, void const *fragment_shader_module_code, bool depth_enable, BRX_GRAPHICS_PIPELINE_COMPARE_OPERATION depth_compare_operation) const
{
    ...
    new_unwrapped_graphics_pipeline->init(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks, render_pass, pipeline_layout, vertex_shader_module_code_size, vertex_shader_module_code, fragment_shader_module_code_size, fragment_shader_module_code, depth_enable, depth_compare_operation);
