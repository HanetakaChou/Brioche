...
void brx_vk_device::init(void *wsi_connection, bool support_ray_tracing)
{
#if defined(__GNUC__)
...
#error Unknown Compiler
    ...
    uint32_t new_upload_queue_queue_index = static_cast<uint32_t>(-1);
    {
        ...
        // D3D11SwapChain::CreatePresenter
        {
            ...
            brx_xcb_connection_T *const brx_xcb_connection = static_cast<brx_xcb_connection_T *>(wsi_connection);
#endif
#else
#error Unknown Platform
#endif
...
#error Unknown Compiler
            ...
            for (uint32_t queue_family_index = 0U; queue_family_index < queue_family_property_count; ++queue_family_index)
            {
                ...
                if ((queue_family_property.queueFlags & VK_QUEUE_GRAPHICS_BIT) &&
                    ...
                    true
#else
                    pfn_vk_get_physical_device_xcb_presentation_support(this->m_physical_device, queue_family_index, brx_xcb_connection->m_connection, brx_xcb_connection->m_visual_id)
...
brx_graphics_queue *brx_vk_device::create_graphics_queue() const
{
    ...
    void *new_brx_graphics_queue_base = mcrt_malloc(sizeof(brx_vk_graphics_queue), alignof(brx_vk_graphics_queue));
...
brx_upload_queue *brx_vk_device::create_upload_queue() const
{
    ...
    void *new_brx_upload_queue_base = mcrt_malloc(sizeof(brx_vk_upload_queue), alignof(brx_vk_upload_queue));
...
{
    void *new_unwrapped_graphics_command_buffer_base = mcrt_malloc(sizeof(brx_vk_graphics_command_buffer), alignof(brx_vk_graphics_command_buffer));
...
{
    void *new_unwrapped_upload_command_buffer_base = mcrt_malloc(sizeof(brx_vk_upload_command_buffer), alignof(brx_vk_upload_command_buffer));
...
brx_fence *brx_vk_device::create_fence(bool signaled) const
{
    ...
    void *new_brx_fence_base = mcrt_malloc(sizeof(brx_vk_fence), alignof(brx_vk_fence));
...
{
    void *new_unwrapped_descriptor_set_layout_base = mcrt_malloc(sizeof(brx_vk_descriptor_set_layout), alignof(brx_vk_descriptor_set_layout));
...
brx_pipeline_layout *brx_vk_device::create_pipeline_layout(uint32_t descriptor_set_layout_count, brx_descriptor_set_layout const *const *brx_descriptor_set_layouts) const
{
    ...
    void *new_brx_pipeline_layout_base = mcrt_malloc(sizeof(brx_vk_pipeline_layout), alignof(brx_vk_pipeline_layout));
...
brx_descriptor_set *brx_vk_device::create_descriptor_set(brx_descriptor_set_layout const *descriptor_set_layout, uint32_t unbounded_descriptor_count)
{
    void *new_unwrapped_descriptor_set_base = mcrt_malloc(sizeof(brx_vk_descriptor_set), alignof(brx_vk_descriptor_set));
...
brx_render_pass *brx_vk_device::create_render_pass(uint32_t color_attachment_count, BRX_RENDER_PASS_COLOR_ATTACHMENT const *color_attachments, BRX_RENDER_PASS_DEPTH_STENCIL_ATTACHMENT const *depth_stencil_attachment) const
{
    ...
    VkRenderPass new_render_pass = VK_NULL_HANDLE;
    {
        ...
        for (uint32_t color_attachment_index = 0U; color_attachment_index < color_attachment_count; ++color_attachment_index)
        {
            ...
            case BRX_COLOR_ATTACHMENT_FORMAT_R16G16_UNORM:
                ...
                break;
            case BRX_COLOR_ATTACHMENT_FORMAT_R32G32_UINT:
                attachments_description[color_attachment_index].format = VK_FORMAT_R32G32_UINT;
                break;
            case BRX_COLOR_ATTACHMENT_FORMAT_R32G32B32A32_UINT:
                attachments_description[color_attachment_index].format = VK_FORMAT_R32G32B32A32_UINT;
                break;
            default:
                assert(false);
    ...
    void *new_brx_render_pass_base = mcrt_malloc(sizeof(brx_vk_render_pass), alignof(brx_vk_render_pass));
    assert(NULL != new_brx_render_pass_base);

    brx_vk_render_pass *new_brx_render_pass = new (new_brx_render_pass_base) brx_vk_render_pass{new_render_pass, color_attachment_count};
...
brx_graphics_pipeline *brx_vk_device::create_graphics_pipeline(brx_render_pass const *render_pass, brx_pipeline_layout const *pipeline_layout, size_t vertex_shader_module_code_size, void const *vertex_shader_module_code, size_t fragment_shader_module_code_size, void const *fragment_shader_module_code, bool enable_back_face_cull, bool front_ccw, bool enable_depth, BRX_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION depth_compare_operation, bool enable_blend, BRX_GRAPHICS_PIPELINE_BLEND_OPERATION blend_operation) const
{
    void *new_unwrapped_graphics_pipeline_base = mcrt_malloc(sizeof(brx_vk_graphics_pipeline), alignof(brx_vk_graphics_pipeline));
...
{
    void *new_unwrapped_compute_pipeline_base = mcrt_malloc(sizeof(brx_vk_compute_pipeline), alignof(brx_vk_compute_pipeline));
...
brx_frame_buffer *brx_vk_device::create_frame_buffer(brx_render_pass const *brx_render_pass, uint32_t width, uint32_t height, uint32_t color_attachment_count, brx_color_attachment_image const *const *color_attachments, brx_depth_stencil_attachment_image const *depth_stencil_attachment) const
{
    ...
    void *new_brx_frame_buffer_base = mcrt_malloc(sizeof(brx_vk_frame_buffer), alignof(brx_vk_frame_buffer));
...
uint32_t brx_vk_device::get_uniform_upload_buffer_offset_alignment() const
{
...
{
    void *new_unwrapped_uniform_upload_buffer_base = mcrt_malloc(sizeof(brx_vk_uniform_upload_buffer), alignof(brx_vk_uniform_upload_buffer));
...
{
    void *new_unwrapped_staging_upload_buffer_base = mcrt_malloc(sizeof(brx_vk_staging_upload_buffer), alignof(brx_vk_staging_upload_buffer));
...
brx_storage_intermediate_buffer *brx_vk_device::create_storage_intermediate_buffer(uint32_t size) const
{
    void *new_unwrapped_storage_intermediate_buffer_base = mcrt_malloc(sizeof(brx_vk_storage_intermediate_buffer), alignof(brx_vk_storage_intermediate_buffer));
    assert(NULL != new_unwrapped_storage_intermediate_buffer_base);

    brx_vk_storage_intermediate_buffer *new_unwrapped_storage_intermediate_buffer = new (new_unwrapped_storage_intermediate_buffer_base) brx_vk_storage_intermediate_buffer{};
...
void brx_vk_device::destroy_storage_intermediate_buffer(brx_storage_intermediate_buffer *wrapped_storage_intermediate_buffer) const
{
    assert(NULL != wrapped_storage_intermediate_buffer);
    brx_vk_storage_intermediate_buffer *delete_wrapped_storage_intermediate_buffer = static_cast<brx_vk_storage_intermediate_buffer *>(wrapped_storage_intermediate_buffer);

    delete_wrapped_storage_intermediate_buffer->uninit(this->m_memory_allocator);

    delete_wrapped_storage_intermediate_buffer->~brx_vk_storage_intermediate_buffer();
    mcrt_free(delete_wrapped_storage_intermediate_buffer);
}

brx_storage_asset_buffer *brx_vk_device::create_storage_asset_buffer(uint32_t size) const
{
    void *new_unwrapped_storage_asset_buffer_base = mcrt_malloc(sizeof(brx_vk_storage_asset_buffer), alignof(brx_vk_storage_asset_buffer));
    assert(NULL != new_unwrapped_storage_asset_buffer_base);

    brx_vk_storage_asset_buffer *new_unwrapped_storage_asset_buffer = new (new_unwrapped_storage_asset_buffer_base) brx_vk_storage_asset_buffer{};
...
void brx_vk_device::destroy_storage_asset_buffer(brx_storage_asset_buffer *wrapped_asset_storage_buffer) const
{
    assert(NULL != wrapped_asset_storage_buffer);
    brx_vk_storage_asset_buffer *delete_wrapped_storage_asset_buffer = static_cast<brx_vk_storage_asset_buffer *>(wrapped_asset_storage_buffer);

    delete_wrapped_storage_asset_buffer->uninit(this->m_memory_allocator);

    delete_wrapped_storage_asset_buffer->~brx_vk_storage_asset_buffer();
...
{
    void *new_unwrapped_color_attachment_image_base = mcrt_malloc(sizeof(brx_vk_color_attachment_intermediate_image), alignof(brx_vk_color_attachment_intermediate_image));
    assert(NULL != new_unwrapped_color_attachment_image_base);

    brx_vk_color_attachment_intermediate_image *new_unwrapped_color_attachment_image = new (new_unwrapped_color_attachment_image_base) brx_vk_color_attachment_intermediate_image{};
...
void brx_vk_device::destroy_color_attachment_image(brx_color_attachment_image *wrapped_color_attachment_image) const
{
    ...
    brx_vk_color_attachment_intermediate_image *delete_unwrapped_color_attachment_image = static_cast<brx_vk_color_attachment_intermediate_image *>(wrapped_color_attachment_image);
    ...
    delete_unwrapped_color_attachment_image->~brx_vk_color_attachment_intermediate_image();
...
{
    void *new_unwrapped_depth_stencil_attachment_image_base = mcrt_malloc(sizeof(brx_vk_depth_stencil_attachment_intermediate_image), alignof(brx_vk_depth_stencil_attachment_intermediate_image));
    assert(NULL != new_unwrapped_depth_stencil_attachment_image_base);

    brx_vk_depth_stencil_attachment_intermediate_image *new_unwrapped_depth_stencil_attachment_image = new (new_unwrapped_depth_stencil_attachment_image_base) brx_vk_depth_stencil_attachment_intermediate_image{};
...
void brx_vk_device::destroy_depth_stencil_attachment_image(brx_depth_stencil_attachment_image *wrapped_depth_stencil_attachment_image) const
{
    ...
    brx_vk_depth_stencil_attachment_intermediate_image *delete_unwrapped_depth_stencil_attachment_image = static_cast<brx_vk_depth_stencil_attachment_intermediate_image *>(wrapped_depth_stencil_attachment_image);
    ...
    delete_unwrapped_depth_stencil_attachment_image->~brx_vk_depth_stencil_attachment_intermediate_image();
...
brx_storage_image *brx_vk_device::create_storage_image(BRX_STORAGE_IMAGE_FORMAT wrapped_storage_image_format, uint32_t width, uint32_t height, bool allow_sampled_image) const
{
    ...
    void *new_unwrapped_storage_image_base = mcrt_malloc(sizeof(brx_vk_storage_intermediate_image), alignof(brx_vk_storage_intermediate_image));
    assert(NULL != new_unwrapped_storage_image_base);

    brx_vk_storage_intermediate_image *new_unwrapped_storage_image = new (new_unwrapped_storage_image_base) brx_vk_storage_intermediate_image{};
    new_unwrapped_storage_image->init(this->m_device, this->m_pfn_create_image_view, this->m_allocation_callbacks, this->m_memory_allocator, this->m_storage_intermediate_image_memory_pool, unwrapped_storage_image_format, width, height, allow_sampled_image);
...
void brx_vk_device::destroy_storage_image(brx_storage_image *wrapped_storage_image) const
{
    ...
    brx_vk_storage_intermediate_image *delete_unwrapped_storage_image = static_cast<brx_vk_storage_intermediate_image *>(wrapped_storage_image);
    ...
    delete_unwrapped_storage_image->~brx_vk_storage_intermediate_image();
    mcrt_free(delete_unwrapped_storage_image);
}

bool brx_vk_device::is_sampled_asset_image_compression_bc_supported() const
{
    return this->m_physical_device_feature_texture_compression_BC;
}

bool brx_vk_device::is_sampled_asset_image_compression_astc_supported() const
{
    return this->m_physical_device_feature_texture_compression_ASTC_LDR;
}

brx_sampled_asset_image *brx_vk_device::create_sampled_asset_image(BRX_SAMPLED_ASSET_IMAGE_FORMAT wrapped_sampled_asset_image_format, uint32_t width, uint32_t height, uint32_t mip_levels) const
{
    ...
    void *new_brx_sampled_asset_image_base = mcrt_malloc(sizeof(brx_vk_sampled_asset_image), alignof(brx_vk_sampled_asset_image));
    assert(NULL != new_brx_sampled_asset_image_base);

    brx_vk_sampled_asset_image *new_brx_sampled_asset_image = new (new_brx_sampled_asset_image_base) brx_vk_sampled_asset_image{};

    new_brx_sampled_asset_image->init(this->m_device, this->m_pfn_create_image_view, this->m_allocation_callbacks, this->m_memory_allocator, this->m_sampled_asset_image_memory_pool, unwrapped_sampled_asset_image_format, width, height, mip_levels);

    return new_brx_sampled_asset_image;
}

void brx_vk_device::destroy_sampled_asset_image(brx_sampled_asset_image *wrapped_sampled_asset_image) const
{
    assert(NULL != wrapped_sampled_asset_image);
    brx_vk_sampled_asset_image *delete_unwrapped_sampled_asset_image = static_cast<brx_vk_sampled_asset_image *>(wrapped_sampled_asset_image);
    ...
    delete_unwrapped_sampled_asset_image->~brx_vk_sampled_asset_image();
...
brx_sampler *brx_vk_device::create_sampler(BRX_SAMPLER_FILTER wrapped_filter) const
{
    ...
    void *new_brx_sampler_base = mcrt_malloc(sizeof(brx_vk_sampler), alignof(brx_vk_sampler));
...
brx_surface *brx_vk_device::create_surface(void *wsi_window) const
{
...
#error Unknown Platform
    ...
    void *new_brx_surface_base = mcrt_malloc(sizeof(brx_vk_surface), alignof(brx_vk_surface));
    assert(NULL != new_brx_surface_base);

    brx_vk_surface *new_brx_surface = new (new_brx_surface_base) brx_vk_surface{};

    new_brx_surface->init(
        this->m_instance,
...
void brx_vk_device::destroy_surface(brx_surface *wrapped_surface) const
{
    ...
    brx_vk_surface *delete_unwrapped_surface = static_cast<brx_vk_surface *>(wrapped_surface);

    delete_unwrapped_surface->uninit(this->m_instance, pfn_destroy_surface, this->m_allocation_callbacks);

    delete_unwrapped_surface->~brx_vk_surface();
    mcrt_free(delete_unwrapped_surface);
}

brx_swap_chain *brx_vk_device::create_swap_chain(brx_surface *wrapped_surface) const
{
    assert(NULL != wrapped_surface);
    VkSurfaceKHR surface = static_cast<brx_vk_surface *>(wrapped_surface)->get_surface();
    ...
    void *new_brx_swap_chain_base = mcrt_malloc(sizeof(brx_vk_swap_chain), alignof(brx_vk_swap_chain));
    assert(NULL != new_brx_swap_chain_base);

    brx_vk_swap_chain *new_brx_swap_chain = new (new_brx_swap_chain_base) brx_vk_swap_chain{};

    new_brx_swap_chain->init(this->m_device, this->m_physical_device, pfn_get_physical_device_surface_formats, pfn_get_physical_device_surface_capabilities, pfn_get_physical_device_surface_present_modes, pfn_create_swap_chain, pfn_get_swap_chain_images, this->m_pfn_create_image_view, m_allocation_callbacks, surface);
...
void brx_vk_device::destroy_swap_chain(brx_swap_chain *wrapped_swap_chain) const
{
    ...
    brx_vk_swap_chain *delete_unwrapped_swap_chain = static_cast<brx_vk_swap_chain *>(wrapped_swap_chain);
    ...
    delete_unwrapped_swap_chain->~brx_vk_swap_chain();
    mcrt_free(delete_unwrapped_swap_chain);
}

brx_scratch_buffer *brx_vk_device::create_scratch_buffer(uint32_t size) const
{
    void *new_unwrapped_scratch_buffer_base = mcrt_malloc(sizeof(brx_vk_scratch_buffer), alignof(brx_vk_scratch_buffer));
...
void brx_vk_device::get_intermediate_bottom_level_acceleration_structure_size(uint32_t bottom_level_acceleration_structure_geometry_count, BRX_BOTTOM_LEVEL_ACCELERATION_STRUCTURE_GEOMETRY const *wrapped_bottom_level_acceleration_structure_geometries, uint32_t *intermediate_bottom_level_acceleration_structure_size, uint32_t *build_scratch_size, uint32_t *update_scratch_size) const
{
    ...
    for (uint32_t acceleration_structure_geometry_index = 0U; acceleration_structure_geometry_index < bottom_level_acceleration_structure_geometry_count; ++acceleration_structure_geometry_index)
    {
        ...
        VkDeviceAddress const vertex_position_buffer_device_memory_range_base = static_cast<brx_vk_acceleration_structure_build_input_read_only_buffer const *>(wrapped_bottom_level_acceleration_structure_geometry.vertex_position_buffer)->get_device_memory_range_base();
        ...
        VkDeviceAddress const index_buffer_device_memory_range_base = (BRX_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? static_cast<brx_vk_acceleration_structure_build_input_read_only_buffer const *>(wrapped_bottom_level_acceleration_structure_geometry.index_buffer)->get_device_memory_range_base() : 0U;
        ...
        max_primitive_counts[acceleration_structure_geometry_index] = (BRX_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? (wrapped_bottom_level_acceleration_structure_geometry.index_count / 3U) : (wrapped_bottom_level_acceleration_structure_geometry.vertex_count / 3U);
    }
...
brx_intermediate_bottom_level_acceleration_structure *brx_vk_device::create_intermediate_bottom_level_acceleration_structure(uint32_t size) const
{
    void *new_unwrapped_intermediate_bottom_level_acceleration_structure_base = mcrt_malloc(sizeof(brx_vk_intermediate_bottom_level_acceleration_structure), alignof(brx_vk_intermediate_bottom_level_acceleration_structure));
    assert(NULL != new_unwrapped_intermediate_bottom_level_acceleration_structure_base);

    brx_vk_intermediate_bottom_level_acceleration_structure *new_unwrapped_intermediate_bottom_level_acceleration_structure = new (new_unwrapped_intermediate_bottom_level_acceleration_structure_base) brx_vk_intermediate_bottom_level_acceleration_structure{};
...
void brx_vk_device::destroy_intermediate_bottom_level_acceleration_structure(brx_intermediate_bottom_level_acceleration_structure *wrapped_intermediate_bottom_level_acceleration_structure) const
{
    assert(NULL != wrapped_intermediate_bottom_level_acceleration_structure);
    brx_vk_intermediate_bottom_level_acceleration_structure *delete_unwrapped_intermediate_bottom_level_acceleration_structure = static_cast<brx_vk_intermediate_bottom_level_acceleration_structure *>(wrapped_intermediate_bottom_level_acceleration_structure);
    ...
    delete_unwrapped_intermediate_bottom_level_acceleration_structure->~brx_vk_intermediate_bottom_level_acceleration_structure();
    mcrt_free(delete_unwrapped_intermediate_bottom_level_acceleration_structure);
}

void brx_vk_device::get_non_compacted_bottom_level_acceleration_structure_size(uint32_t bottom_level_acceleration_structure_geometry_count, BRX_BOTTOM_LEVEL_ACCELERATION_STRUCTURE_GEOMETRY const *wrapped_bottom_level_acceleration_structure_geometries, uint32_t *non_compacted_bottom_level_acceleration_structure_size, uint32_t *build_scratch_size) const
{
    assert(NULL != wrapped_bottom_level_acceleration_structure_geometries);
...
brx_non_compacted_bottom_level_acceleration_structure *brx_vk_device::create_non_compacted_bottom_level_acceleration_structure(uint32_t size) const
{
    void *new_unwrapped_non_compacted_bottom_level_acceleration_structure_base = mcrt_malloc(sizeof(brx_vk_non_compacted_bottom_level_acceleration_structure), alignof(brx_vk_non_compacted_bottom_level_acceleration_structure));
    assert(NULL != new_unwrapped_non_compacted_bottom_level_acceleration_structure_base);

    brx_vk_non_compacted_bottom_level_acceleration_structure *new_unwrapped_non_compacted_bottom_level_acceleration_structure = new (new_unwrapped_non_compacted_bottom_level_acceleration_structure_base) brx_vk_non_compacted_bottom_level_acceleration_structure{};
...
void brx_vk_device::destroy_non_compacted_bottom_level_acceleration_structure(brx_non_compacted_bottom_level_acceleration_structure *wrapped_non_compacted_bottom_level_acceleration_structure) const
{
    assert(NULL != wrapped_non_compacted_bottom_level_acceleration_structure);
    brx_vk_non_compacted_bottom_level_acceleration_structure *delete_unwrapped_non_compacted_bottom_level_acceleration_structure = static_cast<brx_vk_non_compacted_bottom_level_acceleration_structure *>(wrapped_non_compacted_bottom_level_acceleration_structure);
    ...
    delete_unwrapped_non_compacted_bottom_level_acceleration_structure->~brx_vk_non_compacted_bottom_level_acceleration_structure();
...
brx_compacted_bottom_level_acceleration_structure_size_query_pool *brx_vk_device::create_compacted_bottom_level_acceleration_structure_size_query_pool(uint32_t query_count) const
{
    ...
    void *new_unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool_base = mcrt_malloc(sizeof(brx_vk_compacted_bottom_level_acceleration_structure_size_query_pool), alignof(brx_vk_compacted_bottom_level_acceleration_structure_size_query_pool));
...
brx_compacted_bottom_level_acceleration_structure *brx_vk_device::create_compacted_bottom_level_acceleration_structure(uint32_t size) const
{
    void *new_unwrapped_compacted_bottom_level_acceleration_structure_base = mcrt_malloc(sizeof(brx_vk_compacted_bottom_level_acceleration_structure), alignof(brx_vk_compacted_bottom_level_acceleration_structure));
    assert(NULL != new_unwrapped_compacted_bottom_level_acceleration_structure_base);

    brx_vk_compacted_bottom_level_acceleration_structure *new_unwrapped_compacted_bottom_level_acceleration_structure = new (new_unwrapped_compacted_bottom_level_acceleration_structure_base) brx_vk_compacted_bottom_level_acceleration_structure{};
...
void brx_vk_device::destroy_compacted_bottom_level_acceleration_structure(brx_compacted_bottom_level_acceleration_structure *wrapped_compacted_bottom_level_acceleration_structure) const
{
    assert(NULL != wrapped_compacted_bottom_level_acceleration_structure);
    brx_vk_compacted_bottom_level_acceleration_structure *delete_unwrapped_compacted_bottom_level_acceleration_structure = static_cast<brx_vk_compacted_bottom_level_acceleration_structure *>(wrapped_compacted_bottom_level_acceleration_structure);
    ...
    delete_unwrapped_compacted_bottom_level_acceleration_structure->~brx_vk_compacted_bottom_level_acceleration_structure();
...
{
    void *new_unwrapped_top_level_acceleration_structure_instance_upload_buffer_base = mcrt_malloc(sizeof(brx_vk_top_level_acceleration_structure_instance_upload_buffer), alignof(brx_vk_top_level_acceleration_structure_instance_upload_buffer));
...
{
    void *new_unwrapped_top_level_acceleration_structure_base = mcrt_malloc(sizeof(brx_vk_top_level_acceleration_structure), alignof(brx_vk_top_level_acceleration_structure));
