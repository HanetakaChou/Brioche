...
public:
    ...
    void init(uint32_t support_ray_tracing, uint32_t descriptor_set_binding_count, BRX_DESCRIPTOR_SET_LAYOUT_BINDING const *descriptor_set_bindings, uint32_t max_per_stage_descriptor_storage_buffers, uint32_t max_per_stage_descriptor_sampled_images, uint32_t max_descriptor_set_storage_buffers, uint32_t max_descriptor_set_sampled_images, PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks);
    void uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks);
    ~brx_vk_descriptor_set_layout();
...
public:
    ...
    void init(bool support_ray_tracing, brx_descriptor_set_layout const *descriptor_set_layout, uint32_t unbounded_descriptor_count, PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks);
    void uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks);
    ~brx_vk_descriptor_set();
...
public:
    brx_vk_render_pass(VkRenderPass render_pass, uint32_t color_attachment_count);
