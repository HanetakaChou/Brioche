...
brx_vk_color_attachment_intermediate_image::brx_vk_color_attachment_intermediate_image() : m_image(VK_NULL_HANDLE), m_allocation(VK_NULL_HANDLE), m_image_view(VK_NULL_HANDLE)
{
}

void brx_vk_color_attachment_intermediate_image::init(VkDevice device, PFN_vkCreateImageView pfn_create_image_view, VkAllocationCallbacks const *allocation_callbacks, VmaAllocator memory_allocator, VmaPool color_transient_attachment_image_memory_pool, VmaPool color_attachment_sampled_image_memory_pool, BRX_COLOR_ATTACHMENT_IMAGE_FORMAT wrapped_color_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
	VkFormat format;
...
void brx_vk_color_attachment_intermediate_image::uninit(VkDevice device, PFN_vkDestroyImageView pfn_destroy_image_view, VkAllocationCallbacks const *allocation_callbacks, VmaAllocator memory_allocator)
{
...
brx_vk_color_attachment_intermediate_image::~brx_vk_color_attachment_intermediate_image()
{
	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_allocation);
	assert(VK_NULL_HANDLE == this->m_image_view);
...
VkImageView brx_vk_color_attachment_intermediate_image::get_image_view() const
{
	return this->m_image_view;
}

brx_sampled_image const *brx_vk_color_attachment_intermediate_image::get_sampled_image() const
{
	assert(VK_NULL_HANDLE != this->m_image_view);
	return static_cast<brx_vk_sampled_image const *>(this);
}

brx_vk_depth_stencil_attachment_intermediate_image::brx_vk_depth_stencil_attachment_intermediate_image() : m_image(VK_NULL_HANDLE), m_allocation(VK_NULL_HANDLE), m_image_view(VK_NULL_HANDLE)
{
}

void brx_vk_depth_stencil_attachment_intermediate_image::init(VkDevice device, PFN_vkCreateImageView pfn_create_image_view, VkAllocationCallbacks const *allocation_callbacks, VmaAllocator memory_allocator, VmaPool depth_transient_attachment_image_memory_pool, VmaPool depth_attachment_sampled_image_memory_pool, VmaPool depth_stencil_transient_attachment_image_memory_pool, VmaPool depth_stencil_attachment_sampled_image_memory_pool, BRX_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT wrapped_depth_stencil_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
	VkFormat format;
	VmaPool memory_pool;
...
void brx_vk_depth_stencil_attachment_intermediate_image::uninit(VkDevice device, PFN_vkDestroyImageView pfn_destroy_image_view, VkAllocationCallbacks const *allocation_callbacks, VmaAllocator memory_allocator)
{
	assert(VK_NULL_HANDLE != this->m_image_view);
	pfn_destroy_image_view(device, this->m_image_view, allocation_callbacks);
	this->m_image_view = VK_NULL_HANDLE;
...
brx_vk_depth_stencil_attachment_intermediate_image::~brx_vk_depth_stencil_attachment_intermediate_image()
{
	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_allocation);
	assert(VK_NULL_HANDLE == this->m_image_view);
...
VkImageView brx_vk_depth_stencil_attachment_intermediate_image::get_image_view() const
{
	return this->m_image_view;
}

brx_sampled_image const *brx_vk_depth_stencil_attachment_intermediate_image::get_sampled_image() const
{
	assert(VK_NULL_HANDLE != this->m_image_view);
	return static_cast<brx_vk_sampled_image const *>(this);
}

brx_vk_storage_intermediate_image::brx_vk_storage_intermediate_image() : m_image(VK_NULL_HANDLE), m_allocation(VK_NULL_HANDLE), m_image_view(VK_NULL_HANDLE)
{
}

void brx_vk_storage_intermediate_image::init(VkDevice device, PFN_vkCreateImageView pfn_create_image_view, VkAllocationCallbacks const *allocation_callbacks, VmaAllocator memory_allocator, VmaPool storage_intermediate_image_memory_pool, VkFormat unwrapped_storage_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
	VkImageAspectFlags const aspect_mask = VK_IMAGE_ASPECT_COLOR_BIT;
...
void brx_vk_storage_intermediate_image::uninit(VkDevice device, PFN_vkDestroyImageView pfn_destroy_image_view, VkAllocationCallbacks const *allocation_callbacks, VmaAllocator memory_allocator)
{
	if (VK_NULL_HANDLE != this->m_image_view)
	{
		pfn_destroy_image_view(device, this->m_image_view, allocation_callbacks);
...
brx_vk_storage_intermediate_image::~brx_vk_storage_intermediate_image()
{
	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_allocation);
	assert(VK_NULL_HANDLE == this->m_image_view);
}
VkImage brx_vk_storage_intermediate_image::get_image() const
{
	return this->m_image;
}

VkImageView brx_vk_storage_intermediate_image::get_image_view() const
{
	return this->m_image_view;
}

brx_sampled_image const *brx_vk_storage_intermediate_image::get_sampled_image() const
{
	assert(VK_NULL_HANDLE != this->m_image_view);
	return static_cast<brx_vk_sampled_image const *>(this);
}

brx_vk_sampled_asset_image::brx_vk_sampled_asset_image() : m_image(VK_NULL_HANDLE), m_allocation(VK_NULL_HANDLE), m_image_view(VK_NULL_HANDLE), m_mip_levels(static_cast<uint32_t>(-1))
{
}

void brx_vk_sampled_asset_image::init(VkDevice device, PFN_vkCreateImageView pfn_create_image_view, VkAllocationCallbacks const *allocation_callbacks, VmaAllocator memory_allocator, VmaPool sampled_asset_image_memory_pool, VkFormat format, uint32_t width, uint32_t height, uint32_t mip_levels)
{
	VkImageCreateInfo const image_create_info = {
		VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,
		NULL,
...
void brx_vk_sampled_asset_image::uninit(VkDevice device, PFN_vkDestroyImageView pfn_destroy_image_view, VkAllocationCallbacks const *allocation_callbacks, VmaAllocator memory_allocator)
{
	if (VK_NULL_HANDLE != this->m_image_view)
	{
		pfn_destroy_image_view(device, this->m_image_view, allocation_callbacks);
...
brx_vk_sampled_asset_image::~brx_vk_sampled_asset_image()
{
	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_allocation);
	assert(VK_NULL_HANDLE == this->m_image_view);
...
VkImage brx_vk_sampled_asset_image::get_image() const
{
	return this->m_image;
}

VkImageView brx_vk_sampled_asset_image::get_image_view() const
{
	return this->m_image_view;
}

brx_sampled_image const *brx_vk_sampled_asset_image::get_sampled_image() const
{
	return static_cast<brx_vk_sampled_image const *>(this);
}

uint32_t brx_vk_sampled_asset_image::get_mip_levels() const
{
