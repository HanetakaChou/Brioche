...
void brx_vk_graphics_pipeline::init(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks, brx_render_pass const *wrapped_render_pass, brx_pipeline_layout const *wrapped_pipeline_layout, size_t vertex_shader_module_code_size, void const *vertex_shader_module_code, size_t fragment_shader_module_code_size, void const *fragment_shader_module_code, bool enable_back_face_cull, bool front_ccw, bool enable_depth, BRX_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION wrapped_depth_compare_operation, bool enable_blend, BRX_GRAPHICS_PIPELINE_BLEND_OPERATION wrapped_blend_operation)
{
	...
	{
	case BRX_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION_UNKNOWN:
		assert(!enable_depth);
		depth_compare_op = VK_COMPARE_OP_LESS;
		break;
	case BRX_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION_GREATER:
		depth_compare_op = VK_COMPARE_OP_GREATER;
		break;
	case BRX_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION_LESS:
		depth_compare_op = VK_COMPARE_OP_LESS;
		break;
	case BRX_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION_ALWAYS:
	...
	{
	case BRX_GRAPHICS_PIPELINE_BLEND_OPERATION_UNKNOWN:
	{
		assert(!enable_blend);
	...
	break;
	case BRX_GRAPHICS_PIPELINE_BLEND_OPERATION_OVER:
	{
	...
	{
		assert(false);
		src_color_blend_factor = static_cast<VkBlendFactor>(-1);
		dst_color_blend_factor = static_cast<VkBlendFactor>(-1);
		color_blend_op = static_cast<VkBlendOp>(-1);
		src_alpha_blend_factor = static_cast<VkBlendFactor>(-1);
		dst_alpha_blend_factor = static_cast<VkBlendFactor>(-1);
		alpha_blend_op = static_cast<VkBlendOp>(-1);
		color_write_mask = static_cast<VkColorComponentFlags>(-1);
	}
	}

	uint32_t const color_attachment_count = static_cast<brx_vk_render_pass const *>(wrapped_render_pass)->get_color_attachment_count();
